...
export default function DataTable() {
    ...
    useEffect(() => {
        setRowsFiltered(() => {
            const filteredData = rows.filter((r) => (r.nome as string).toUpperCase().includes(filter?.toUpperCase() || '') || (r.municipio.nome as string).toUpperCase().includes(filter?.toUpperCase() || ''));
            return filteredData;
        })
    }, [filter, rows])
    ...
    useEffect(() => {
        fetchDistrito('', { nome: '' }).then((respD) => setRows(respD.data)).catch((error) => {
